...
| **Trang giới thiệu (showcase)** | `gioi-thieu.html` (**v4.x**, mục "Mới trong 4.1–4.3" `#new43` ảnh `img/showcase/v43/` (script chụp: Playwright 375×812 @1.44 → 540×1169 webp), "Mới trong 4.0" ảnh `img/showcase/v4/`, phần còn lại dùng ảnh v3) + lưu trữ `gioi-thieu-v3.html`, `gioi-thieu-v2.html`, `gioi-thieu-v1.html` | `…/gioi-thieu.html` | Landing fullpage scroll-snap, nền particle-net, mục "Mới trong v3.0" (lưới 5 ngành), mục **Lịch sử phiên bản** (`#versions`). Dark-first. Quy ước version lớn: xem mục 7 |
...
├── vendor/tesseract/     # v4.2: tesseract.js 5 + core LSTM + eng.traineddata (≈11 MB) — OCR trên máy, nạp lười ở màn Ảnh → bài học
